...
class IOLoop(Configurable):
    ...
    def current(instance=True):
        ...
        if asyncio is None:
            ...
            try:
                loop = asyncio.get_event_loop()  #获取协程引用
                print('loop 1 =',loop,id(loop))
                print('loop Done')
            ...
            try:
                print('try')
                print('-----------------------------------------')
                for v,k in IOLoop._ioloop_for_asyncio.items():
                    print(v,k)
                print('-------------------------------------------')
                print("LOLoop ====",IOLoop._ioloop_for_asyncio[loop])  #在字典中么有查找到 抛出异常KeyError
                return IOLoop._ioloop_for_asyncio[loop]
            except KeyError:
                print('keyerror')
                if instance:  # 初始化为True  若没有当前IOLOOP的实例而且instance为Ture则创建一个实例
                    print('instance',instance)
                    from tornado.platform.asyncio import AsyncIOMainLoop
                    current = AsyncIOMainLoop(make_current=True)
                    print("asynciomainLoop")
                else:
                    print('else')
                    current = None
        print('currenttttttttttttttt',current)
        print("ok")
        return current
    ...
    def initialize(self, make_current=None):
        print('make_current',make_current)
        if make_current is None:   #判断是否是第一次调用初始化
            if IOLoop.current(instance=False) is None:
                self.make_current()
        elif make_current:
            print('els')
            current = IOLoop.current(instance=False) #return is current_
            print('loop3 ',current,id(current))
            # AsyncIO loops can already be current by this point.
            if current is not None and current is not self:
                print("looppppppppppppppp Done")
...
class PollIOLoop(IOLoop):
    ...
    def initialize(self, impl, time_func=None, **kwargs):
        ...
        if hasattr(self._impl, 'fileno'):
            set_close_exec(self._impl.fileno())        # 文件描述符的close_on_exec属性
        ...
        self._pid = os.getpid()
        self._blocking_signal_threshold = None     # 获得当前线程标识符
        ...
        try:
            while True: # #  #  # 服务器进程正式开始
                ...
                if self._timeouts:  # 将时间已到的定时任务放置到due_timeouts中
                    ...
                    if (self._cancellations > 512 and  # 当超时计数器大于 512 并且 大于 _timeouts 长度一半（ >> 为右移运算， 相当于十进制数据被除 2 ）时，清零计数器，并剔除 _timeouts 中无 callbacks 的任务
                ...
                for i in range(ncallbacks):  # 运行 callbacks 里所有的 calllback
                    self._run_callback(self._callbacks.popleft())
                for timeout in due_timeouts:
                    if timeout.callback is not None:
                        self._run_callback(timeout.callback) #  运行所有已过期任务的 callbac
                # Closures may be holding on to a lot of memory, so allow
                # them to be freed before we go into our poll wait.
                due_timeouts = timeout = None   # 释放内存
                ...
                while self._events:
                    #　 self._events 用来存储epoll_obj.poll()返回的事件，
                    # 也就是哪个fd发生了什么事件{(fd1, event1), (fd2, event2)……}
