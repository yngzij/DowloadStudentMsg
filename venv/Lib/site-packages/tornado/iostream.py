...
class BaseIOStream(object):
    ...
    def __init__(self, max_buffer_size=None,
        ...
        # max_write_buffer_size - 要缓冲的传出数据量; 默认为无限制
        ...
        self.max_buffer_size = max_buffer_size or 104857600  #缓冲区最大长度
        # A chunk size that is too close to max_buffer_size can cause
        # spurious failures.


         #chunk一般都是客户端发送的请求了，
        # 但是我们还需要对这些chunk作进一步操作，
        # 比如这个chunk中可能包含了多个请求，
        # 如何把请求分离？(每个请求首部的结束符是
        # b'\r\n\r\n')，
        # 这里就用到read_until来分离请求并设置callback了。
        # 同时会将被分离的请求数据从_read_buffer中移除。
        #然后就是将callback以及他的参数(被分离的请求数据)
        #添加至IOLoop._callbacks中，等待下一次IOLoop的执行，届时会迭代_callbacks并执行回调函数
        #tornado是水平触发，
        # 所以假如读完一次chunk后系统缓存区中依然还有数据，
        ...
        self.error = None
        self._read_buffer = bytearray()   #_read_buffer表示读缓冲
        self._read_buffer_pos = 0    #末尾位置
        self._read_buffer_size = 0   # 缓冲区已经使用的长度
        self._user_read_buffer = False
        self._after_user_read_buffer = None
        self._write_buffer = _StreamBuffer()
        #读的过程中也会检测总尺寸是否大于我们设定的最大缓冲尺寸
        ...
        self._close_callback = None   #关闭套接字时候需要执行的回调函数
        self._connect_callback = None  #连接时候执行的回调函数
...
##########################################################################################
    ...
    # 达到这些字节就会触发并运行回调函数(
    # 当然这些回调函数不是立刻运行，
    # 而是被送到ioloop中的_callbacks中)，
    # 该方法主要是用来读取包含content-length或者分块传输编码的具有主体信息的请求或者响应。

    def read_bytes(self, num_bytes, callback=None, streaming_callback=None,
                   partial=False):
        #如果partial为true，则只要我们返回任何字节
        # （但从不超过num_bytes），就会运行回调
        #read_bytes和read_until_close这两个方法都有streaming_callback这个参数
        #假如指定了该参数
    ...
    def read_into(self, buf, callback=None, partial=False):
        """Asynchronously read a number of bytes.
        #如果partial为true，则在读取任何字节后立即运行回调。否则，它buf在完全填充读取数据时运行。
        buf必须是一个可写入的缓冲区，数据将被读入。
        如果给出了回调，它将以读取字节数作为参数运行; 如果没有，这个方法返回一个Future。
        ...
        n = len(buf)
        if available_bytes >= n:  #有足够的空间存储
            end = self._read_buffer_pos + n #尾指针位置
        ...
        self._read_bytes = n  #读取的尾地址
        self._read_partial = partial   #已经读取的部分
...
#####################################################
    #异步读取套接字中的所有数据，直到它关闭
    #read_until_close则是主要被用在非持久连接上
    # 因为非持久连接响应的结束标志就是连接关闭。
...
# 将callback封装后添加至ioloop._callbacks中
        ...
        try:
            ...
            while not self.closed():
                ...
                if self._read_to_buffer() == 0:
                    # 从系统缓冲中读一个chunk并将其添加至_read_buffer中，然后返回chunk的大小，如果无数据则返回0
                    break
    ...
    def _run_read_callback(self, size, streaming):
        ...
        if callback is not None: # 这里将_read_future进行set_result
            assert (self._read_future is None) or streaming
            self._run_callback(callback, result)
            # 将后者作为前者的参数，然后将前者进行封装后添加至IOLoop._callbacks中
    ...
    def _try_inline_read(self):
        ...
        # 检查字符流回调，
        # 如果调用read_bytes和read_until_close并指定了streaming_callback参数就会造成这个回调

        pos = self._find_read_pos()
        # 尝试在_read_buffer中找到分隔符的位置。找到则返回分隔符末尾所处的位置，如果不能，则返回None。
        ...
        try:
            pos = self._read_to_buffer_loop()
            # 从系统缓冲中读取一个chunk，检查是否含有分隔符，
            # 没有则继续读取一个chunk，合并两个chunk，
            # 再次检查是否函数分隔符……
...
################################
    ...
    def _read_from_buffer(self, pos):
        ...
        self._read_partial = False #部分读取
        self._run_read_callback(pos, False)

    def _find_read_pos(self):
        # 尝试在_read_buffer中寻找分隔符。找到则返回分隔符末尾所处的位置，如果不能，则返回None
        ...
        if (self._read_bytes is not None and
            ...
            if self._read_buffer:  # 查看_read_buffer中是否有之前未处理的数据
                ...
                if loc != -1: # 在_read_buffer的首项中找到了
                    loc -= self._read_buffer_pos #在当前的位置
                    delimiter_len = len(self._read_delimiter)#分割符的长度
                ...
                self._check_max_bytes(self._read_delimiter,
                                      self._read_buffer_size)
        elif self._read_regex is not None:  #匹配正则表达式
    ...
    def _consume(self, loc):
        ...
        if loc == 0:
            # 将self._read_buffer 的首项改为 原首项[loc:] ，然后返回 原首项[:loc]
            return b""
    ...
    def _add_io_state(self, state):
        ...
        if self._state is None:
            ...
            with stack_context.NullContext():     # self._handle_events 是根据events选择对应的处理函数，在这里我们假设处理函数是_handle_read

                self.io_loop.add_handler(
                    self.fileno(), self._handle_events, self._state)
                # 为对应socket的文件描述符添加事件及其处理函数,
...
class IOStream(BaseIOStream):
    ...
    def __init__(self, socket, *args, **kwargs):
        print("iostream args", args)
        print("iostream  kwarssss", kwargs)
